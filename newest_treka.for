...
C NOTE: THE ABOVE ADDRESS IS NO LONGER VALID, SORRY KIDDIES            *
C=======================================================================
C                                                                      *
...
C***** MODULE NOTE: A.FOR (KLIN <- HANDWRITTEN) *****
     ...
     *ICLOZ,IONNO,IPLANZ,IRUNZ,IDEVZ,IDRIZ,IMAXZ
      COMMON /R/IPBSC(4),IBPSB(8),IBPSS(2),IBPSE(25)
      ...
      DATA CMAND/'IN','MA','TR','CO','PH','TO','GO','CA','EA','BO',
     *'YA','DE','HE','MI','RE','RA','CE','AT','DA','CR','SP','EN',
     *'NO','SU','HN','VE'/
      DATA NO/'N'/
      ITOM=12345
...
129   IT=IT+1
      ...
      CALL SAURON
234   WRITE(L,10)(INAME(IQ0,IT),IQO=1,3)
...
666   IF(LOCKT(IT).NE.1)GO TO 668
      ...
      GO TO 6661
6660  WRITE(L,667)
667   FORMAT(' ENTER RANGE WHICH TORP TO BECOME MINE - FROM 1.5 TO 10')
...
1440  CONTINUE
      ...
      IF(IBPSS(I).EQ.IS)WRITE(L,146)NDEAD(I),IBASE(I)
...
170   CONTINUE
      ISURR(IT)=1
      WRITE(L,133)
133   FORMAT(' PLEASE PRESENT YOUR SURRENDER TO NEAREST ENEMY VESSEL')
      GO TO 234
...
91    FORMAT (/' STARBASE',I3,' CALLING  ', 3A4, /' DUE TO YOUR COWARDL
     *Y SURRENDER, WE HAVE LOST FACE. '/' FOR THIS ACT YOURE DEMOTED TO
...
90    FORMAT('1',15('-'),'  KEEP ON TREKKIN''   ',15('-')/'1    IF ANY E
     ...
     *'CAREOF DAN GAHLINGER VIA,'/' DAN@ZEROFUSION.COM -',5X)
...
331   IF(IGOL(1).NE.'H')GO TO 334
      WRITE(L,335)
335   FORMAT( ' ENTER STRING OF CHARACTERS INDICATING THE SEQUENCE IN W
     ...
     *D/OR "D" TO ENGAGE A DEFLECT BEAM. '/' EG. IF YOU WISH TO FIRE
     ...
     *' TYPE "TYBMPD". '/' NOTE YOU DON''T HAVE TO USE ALL THE CHA
     *RACTERS-'/' COMMANDS SUCH AS "M" OR "PT" OR "BY" ARE POSSIBLE.')
      WRITE(L,336)
...
337   FORMAT(' OTHER FORMS--'/' EG. B(M)(PT) -IF THE BOARD IS SUCCESSFUL
     *, WE MOVE ELSE WE FIRE PHASERS AND TORPEDOES. '/'  THE BACKUS-N
...
C
      ...
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(80)
      COMMON /D/DFLCT(4),DFLCK(8),DFLCB(8)
      COMMON /E/PHASR(4),TWARP(4),IPHOT(4),NDEAD(4),ISURR(4)
      COMMON /I/IONK(8),ISPOK(8),ION(4),ISPOT(4)
      COMMON /N/INAME(3,4),IENM2(4,8),ISIDE(3,2),IBASE(2)
...
1341  LAST=IBPSC(I)+4
134   CONTINUE
      IF(IJ(l)+IJ(2).EQ.0)GO TO 98
      IF(ISHAK.EQ.1)GO TO 100
      IF(II(l).NE.0.AND.II(2)+I3.EQ.0)GO TO 139
...
C old code was RT=RAN(ISHRT,ISHIT)
      RT=(RAND(0)*(ISHIT-ISHRT))+ISHRT
      ITOM=IFIX(RT*10000.)
...
C
     ...
     *IGNOR,IO,IGOL(80)
      COMMON /L/IENTR(4),IENTC(4),IKLNR(8),IKLNC(8),IGLER(25),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /M/MAP(60,60),IBLK,IENMI(8),IEE(4),IGLE,IM(4),III,ISTAR
