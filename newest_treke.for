...
C
     ...
     *IGNORE,IO,IGOL(80)
      ...
      COMMON /K/IWHO(8),DIST(B),KIIIZ,KIIIR,KIIID,KIIIG,IGOCO(8)
      COMMON /L./IENTR(4),IENTC(4),IKLNR(S),IKLNC(B),IGL-ER(25),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /M/MAP(60,60),IBLK,IENM1(9),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /N/INAME(3,4),IENM2(4,8),ISIDE(3,2),IBASE(2)
      COMMON /Q/IARMZ(6),INDUZ,IXRYZ,IMAGZ,IMAGRZ,IABSZ,ISTAZ,INVIZ(4),
     *ICLOZ,IONND,IPLANZ,IRUNZ,IDEVZ,IDRIZ,IMAXZ
...
11372 FORMAT(' MIN=I,I,' I7=',I,' I8=',I,' DISTP=',F)
...
10801 I81=I8
      ...
      itvll=1
      rtvll=1.5
      CALL HORTA(I71,IB1,IKLNR(I),IKLNC(I),
      *rtvll,rtzro,itzro,rtzro,itvll,rtzrooitzro)
      IF(MIN.NE.I+4.AND.MIN.NE.19.AND.MIN.LT.21)GO TO 10801
1080  MAP(IKLNR(I),IKLNC(I))=I8LK
...
C -WHERE THE SHADOWS LIE
      ...
      COMMON /C/L,A,B,I,NA,IViI7,IS,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG
     *IGNORE,IO,IGOL(80)
      COMMON /D/DFLCT(4),DFLCK(8),DFLCB(2)
      COMMON /I/IONK(8),ISPOK(8),ION(4),ISPOT(4)
      COMMON /K/IWHO(8),DIST(8),KIIIZ,KIIIR,KIIID,KIIIG,IGOCO(S)
      COMMON /L/IENTR(4),IENTC(4),IKLNR(8),IKLNC(8),IGLER(25),TGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),Ll2C(5),IGO(4),MINES
...
1006  CALL RANDO(J,7,9)
      ...
      RAD=FLOAT(J)
1005  AJUST=0.0
1009  IF(IBPSB(l).EQ.0.OR.IGOCO(l).EQ.0)GO TO 600
      DISTP=100.
      IVV=0
      DO 601 IV=1,4
      IF(ICHOE(IV).NE.1.OR.(IBPSC(IV)+l)/2.NE.IBPSB(I))GO TO 601
...
600   IV=MM-IKLNR(I)
      ...
      BERNG=ANG(IV,IVV)
      BERNG=BERNG+AJUST
      IF(BERNG.LT.O)BERNG=BERNG+360.
      IF(BERNG.GT.360.)BERNG=BERNG-360.
      DO 504 17=1,4
      IF(ICHOE(I7).NE.1)GO TO 504
      CALL DUNSEL(IKLNR(1),IKLNC(I),RAD,BERNG)
504   CONTINUE
      MORDOR=.FALSE.
      RETURN
      CALL RANDO(J,7,9)
      ...
      DO 10010 J=1,4
10010 INVIS(J)=0
      IF(DISTP.LE.10..OR.KIIIR.NE.1)KIIIR=2
      IF(A-DFLCK(2).GT.20.)KIIIR=3
      IV=IV-KIIIR
      I71=MIN+4
      GO TO (1003,10031,1004),KIIIR
...
1007  RAD=11.
      KIIIG=1
      IF(DFLCK(4)-A.LT.25.)KIIIG=2
      IF(KIIIG.EQ.1)GO TO 1005
1008  AJUST=180.
      GO TO 1009
1011  RAD=12.
...
110   RAD=IMAXZ
      ...
      IF(KIIIZ.EQ.100)GO TO 124
      IF(DISTP.LE.KIIIZ)GO TO 125
      CALL RANDO(I7,1,360)
      AJUST=I7
      GO TO 1009
125   KIIIZ=100
...
C
     ...
     *ICLOZ,IONNO,IPLANZ,IRUNZ,IDEVZ,IDRIZ,IMAXZ
      ...
      MIN=0
      DO 1 IVV=1,4
      IF(ICHOE(IVV).EQ.O.OR.ICHOE(IVV).EQ.3)GO TO 1
      IF(J3.EQ.(IBPSC(IVV)+I)/2)GO TO 1
      AJUST=DI(Jl,J2,IENTR(IVV),IENTC(IVV))
...
1     CONTINUE
      ...
      AJUST=DI(Jl,J2,IBASR(IVV),IBASC(IVV))
...
2     CONTINUE
      ...
      AJUST=DI(Jl,J2,IKLNR(IVV),IKLNC(IVV))
...
4     IF(NUMOUT.EQ.0)RETURN
      ...
      AJUST=DI(JI,J2,IGLER(IVV),IGLEC(IVV))
...
C
      ...
      INTEGER EDIBLE
      COMMON /C/L,AB,I,NA,IV,I7,IB,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,I0,IGOL(60)
      COMMON /D/DFLCT(4),DFLCK(B),DFLCB(2)
      COMMON /I/IONK(B),ISPOK(B),ION(4),ISPOT(4)
      COMMON /K/I.WHO(B),DIST(B),KIIIZ,KIIIR,KIIID,KIIIG,IGOCO(8)
      COMMON /L/IENTR(4),IENTC(4),IKLNR(B),IKLNC(8),IGLER(25),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGD(4),MINES
      COMMON /M/MAP(60,60),IBLK,IENM1(8)sIEE(4),IGLE,IM(4),III,ISTAR
      COMMON /N/INAME(3,4),IENM2(4tB)jISIDE(3,2),IBASE(2)
      COMMON /T/ICHOE(4),ICHOS(8),ICHOB(2)
      COMMON /V/IWEB(2),IWEBZ,INVIS(4)
         COMMON /TOM/ITOM
...
529   DFLCB(IV)=-1.
      ...
      FORMAT(' THE DOOMSDAY MACHINE HAS EATEN ',3A4,' STARBASE',13)
5190  CONTINUE
519   CONTINUE
      GO TO 1000
...
517   ISTAT=2
      ...
      IV=IVV+4
      WRITE(IV,527)(INAME(IGO,MIN),IGO=1,3)
...
530   MIN=MIN-14
      CALL BOOM(MIN)
      DO 533 IVV=1,4
      IF(ICHOE(IVV).EQ.0)G0 TO 533
      IV=IVV+4
      WRITE(IV,534)MIN
534   FORMAT(' THE DOOMSDAY MACHINE HAS EATEN EAGLE',13)
533   CONTINUE
      GO TO 1000
1000  RETURN
      END
...
C
      ...
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(80)
      COMMON /L/IEN'TR(4),IENTC(4),IKLNR(8),IKLNC(8),IGLER(25),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),Ll2R(5),LI2C(5),IGO(4),MINES
      ...
      DO 3 IK=1,2
      IF(MM.EQ.IBASR(IK).AND.MN.EQ.IBASC(IK).AND.IJ(IBPSS(IK)).EQ.0)RETURN
3     CONTINUE
      EDIBLE=1
      RETURN
...
C
     ...
     *IGNORE,IO,IGOL(80)
      COMMON /D/DFLCT(4),DFLCK(8),DFLCB(2)
      COMMON /L/IENTR(4),IENTC(4),IKLNR(8),IKLNC(8),IGLER(25),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),Ll2C(O),100(4),MINES
      COMMON /N/INAME(3,4),IENM2(8),ISIDE(3,2),IBASE(2)
      COMMON /T/ICHOE(4),ICHOS(8),ICHOB(2)
         COMMON /TOM/ITOM
...
C
     ...
     *IGNORE,IO,IGOL(80)
      COMMON /L/IENTR(4),IENTC(4),IKLNR(8),IKLNC(8),IGLER(25),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGD(4),MINES
      COMMON /M/MAP(60,60),IBLK,IENMI(8),IEE(4),IGLE,IM(4),III,ISTA
      COMMON /G/IARMZ(6),INDUZ,IXRYZ,IMAGZ,IMAGRZ,IABSZ,ISTAZ,INVIZ(4),
     *ICLOZ,IONNO,IPLANZ,IRUNZ,IDEVZ,IDRIZ,IMAXZ
        COMMON /TOM/ITOM
85    IF(IARMZ(4).EQ.0)GO TO 81
      IF(IMAGZ.EQ.0)G0 TO 81
      MIN=0
      DO 86 N=I,IMAGZ
87    CALL RANDO(IV,-IMAGRZ,IMAGRZ)
      CALL RANDO(IVV,-IMAGRZ,IMAGRZ)
      MIN=MIN+1
      IF(MIN.GT.20)GO TO 61
      IV=IKLNR(6)+IV
      IVV=IKLNC(6)+IVV
...
86    MAP(IV,IVV)=IENM1(6)
      ...
      IF(.NOT.WODEN(IV,IVV,MAP(lV,IVV)))GO TO 83
      IKLNR(6)=IV
      IKLNC(6)=IVV
      GO TO 88
83    CONTINUE
...
C
     ...
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /M/MAP(60,60),IBLK,IENM1(8),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /T/ICH0E(4),ICHOS(8),ICHOB(2)
      COMMON /U/LAUNCH,NUMOUT,NUME(2)
      CALL SIMAK(J1,J2)
      ISTAT=0
      I=0
...
6     I71=I7
     ...
     *rtvll,rtzro,itzro,rtzro,IGNORE,itzro,itzro)
...
10    I=0
      GO TO 11
2     CONTINUE
      IF(LAUNCH.LT.25.AND.ICHOS(B).NE.0)CALL LEGUIN(JI,J2)
      RETURN
      END
...
14    AJUST=100.
      ...
      DISTP=DI(IENTR(I7),IENTC(17),I71,I81)
...
C
      ...
      COMMON /A/IT,IS,II(2),IJ(2),I3,JS,ISHAK,NOSTOP
      COMMON /C/L,A,B,IN,A,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,I0,IGOL(80)
      COMMON /D/DFLCT(4),DFLCK(B),DFLCB(2)
      COMMON /L/IENTR(4),IENTC(4),IKLNR(B),IKLNC(B),IGLER(25),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGD(4),MINES
      COMMON /M/MAP(60,60),IBLKiIENM1(8),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /N/INAME(3,4),IENM2(4,8),ISIDE(3,2),IBASE(2)
      COMMON /R/IBPSC(4),IBPSB(8),IBPSS(2),IBPSE(25)
      ...
      I81=1
      I8=LAUNCH+1
107   IF(IS.GT.25)00 TO 113
      I71=IFIX(1.6*SIN(DISTP+AJUST))+IKLNR(8)
...
123   DO 124 MIN=1,4
      ...
      N=MIN+4
      WRITE(N,103)IB,(INAME(IGO,JTK),IGO=1,3),IB,
     *(INAME(IQ0,JTK),IQ0=I,3)
103   FORMAT( ' EAGLE', 13, ' CRASHED INTO ', 3A4, ' ON TAKE-OFF'/' EAGLE',I3
     *,' DESTROYED'/IXi3A4,' DAMAGE REPORT-')
124   CONTINUE
      CALL RANDO(IV,1,100)
      ...
      GO TO 106
102   DO 125 J1=1,4
      IF(ICHOE(J1).EQ.0)G0 TO 125
      J2=J1+4
      WRITE(J2,104)I8
104   FORMAT(' EAGLE',13,' DESTROYED ON COLLISION WITH A MINE')
125   CONTINUE
      MAP(I71,IVV)=IBLK
      MINES=MINES-1
      NUMOUT=NUMOUT-1
