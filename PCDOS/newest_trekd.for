...
C
     ...
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      ...
      COMMON /NIINAME(3,4),IENM2(4,8),ISIDE(3,2),IBASE(2)
...
766   IV=I7-7
      ...
      DO 780 N=IV,IV1
      DO 780 J=IVV,IZA
      IF((8B-J)**2+(I7-N)**2.LE.49)MAP(N,J)=IBLK
780   CONTINUE
      DO 769 N=1,8
      IF(ICHOS(N).EQ.0)GO TO 769
      IF(N.EQ.7)GO TO 769
      IF((IKLNC(N)-I8)**2+(IKLNR(N)-I7)**2.GT.49)GO TO 76
      ISPOK(N)=0
      CALL TOPHET(N)
769   CONTINUE
...
7690  DO 55 KOENIG=1,4
      ...
      IV=J+4
      WRITE(IV,772)(INAME(IQ0,KDENIG),IQ0=1,3)
...
C
      ...
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG
     *IGNORE,I0,IGOL(BO)
      COMMON /I/IONK(B),ISPOK(8),ION(4),ISPOT(4)
      COMMON /L./IENTR(4),IENTC(4),IKLNR(8),IKLNC(8),IGLER(25),IGLEC(25),
...
C
      SUBROUTINE STYX
      COMMON /A/IT,IS,II(2),IJ(2),13#JS,ISHAK,NOSTOP
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(80)
      COMMON /D/DFLCT(4),DFLCK(8),DFLCB(2)
     ...
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /M/MAP(60,60),IBLK,IENM1(8),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /IN/INAME(3,4),IENM2(4,8),ISIDE(3,2),IBASE(2)
      COMMON /T/ICHOE(4),ICHOS(El),ICHOB(2)
      COMMON /V/IWEB(2),IWEBZ,INVIS(4)
              COMMON /TOM/ITOM
      CALL RANDO(IV,1,33)
      IVV=I+4
      J=(I+1)/2
2380  WRITE(IVV,238)IBASE(J),(INAME(IQO,I),100=1,3)
...
C
      SUBROUTINE BELIAL
      COMMON /C/L,A,B,I,NA,IV,I7,IB,N,DISTPiAJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(80)
      COMMON /E/PHASR(4),TWARP(4),IPHOT(4),NDEAD(4),ISURR(4)
      COMMON /N/INAME(3,4),IENM2(4,8),ISIDE(3,2),IBASE(2)
      COMMON /T/ICHOE(4),ICHOS(8),ICHOB(2)
...
C
      ...
      OPEN (UNIT=1,DEVICE='DRA0:',ACCESS='RANDIN',MODE='ASCII',
     *FILE='KIRK.DAT',DIRECTORY='010011.D.WORK'.RECORD SIZE=80)
      OPEN(UNIT=2,DEVICE='DRA0:',ACCESS='RANDIN',MODE='ASCII',
     *FILE='KOLOTH.DAT',DIRECTORY='01011.D.WORK',RECORD SIZE=80)
      OPEN(UNIT=1,TYPE='OLD',ACCESS='DIRECT',FORM='FORMATTED',
     *RECORDTYPE='IFIXED',RECORDSIZE=80,readonly,
     *ORGANIZATION='RELATIVE',NAME='SYS$USERS:KIRK.DAT')
      OPEN(UNIT=2,TYPE='OLD',ACCESS='DIRECT',FORM='FORMATTED',
     *RECORDTYPE='IFIXED',RECORDSIZE=80,readonly,
...
C used by vax version
     ...
     *IGNORE,I0,IGOL(80)
      ...
      COMMON /G/ZAP(4),ICOLA(4),IRDWA(4),LOCK(4),ICOIL(4)
...
115   IF(ICHOS(I).EQ.1)I3=I3+1
      DO 119 IVV=1,2
      J=3-IVV
      IF(II(IVV).E1.0.OR.II(J).NE.0)GO TO 119
      CALL MENDEZ(J)
119   CONTINUE
      DO 116 IVV=1,2
...
116   CONTINUE
      ...
      IF(II(1).EQ.0.AND.13.EQ.0)ISHAK=1
      IF(ISHAK.EG.0)GO TO 126
      DO 122 J=1,4
      I=J+4
      IF(ICHQE(J).EQ.1)WRITE(I,6100)
...
754   CONTINUE
      ...
      INVIS(I)=1
127   CONTINUE
890   IF(IARMZ(5).EQ.0)GO TO B9
      IF(ICHOS(6).EQ.0)GO TO 89
      DO 691 I=1,4
      IF(ICHOE(I).EQ.0)GO TO 691
      IF(DI(IKLNC(6),IKLNR(6),IENTC(I),IENTR(l)).GE.ICLOZ)GO TO 891
      INVIZ(I)=1
891   CONTINUE
89    ISTAT=0
      DO 170 I= 1,8
...
C
      ...
      DATA MON/-28580569024#-30711666672,-26969B25216,-33349361600,
     *-26969595840,-28496682944,-2849674B480,-333287505213,-23731888064,
     *-25887629248,-26374102976,-31785377728/
...
C ABOVE LINE MAY GENERATE A FALSE ERROR
      M=I(1)/2 + I(2)/128 + D/2 + A/128 + 2653589
      CALL SETRAN(M)
      DECODE(2,4,D)M
4     FORMAT(I2)
...
14    WRITE(L,15)
15    FORMAT(' AS PASSWORD ENTER NAME OF THE DISCOVERER OF THE SPACE WARP')
      GO TO 8
12    WRITE(L,13)
...
11    FORMAT(' AS PASSWORD ENTER KIRK''S MIDDLE NAME')
      GO TO 8
9     WRITE(L,6)
6     FORMAT(' AS PASSWORD ENTER NAMES OF SIGNERS OF ORGANIAN PEACE TREATY')
      GO TO 8
16    WRITE(L,17)
17    FORMAT(' AS PASSWORD ENTER MUDD''S FULL NAME')
      GO TO 6
18    WRITE(L,19)
19    FORMAT('AS PASSWORD ENTER NAME OF SHIP WITH STAR FLEET ID NUMBER NCC-3810')
      GO TO 8
20    WRITE(L,21)
...
C               - MENDEZ
      ...
      SUBROUTINE MENDEZ(J)
      COMMON /A/ITPIS,II(2),IJ(2),I3,JS,ISHAK,NOSTOP
      COMMON /B/NDEAB(2),IPHOB(2),IONB(2),IGOB(2),IFIB(2)
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(80)
      COMMON /L/IENTR(4),IENTC(4),IKLNR(8),IKLNC(S),IGLER(25),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /M/MAP(60,60),IBLK,IENM1(8),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /N/INAME(3,4),IENM2(4,8),ISIDE(3,2),IBASE(2)
      COMMON /T/ICHOE(4),ICHOS(8),ICHOB(2)
      ...
      IF(II(1).EQ.0)I=3
      IF(II(2).EQ.0)N=2
      IF(J.EQ.2.AND.ICHOB(l).EQ.1)I=3
      DO 1 MIN=I,N
...
C
     ...
     *IGNORE,IO,IGOL(80)
      COMMON /D/DFLCT(4),DFLCK(8),DFLCB(2)
      COMMON /I/IDNK(B),ISPOK(B),IDN(4),ISPOT(4)
      COMMON /K/IWHO(S),DIST(B),KILLZ,KILLR,KILLD,KILLG,IGOCO(8)
      COMMON /LIIENTR(4),IENTC(4),IKLNR(E3),IKLNC(S),IGLER(25),IGLEC(25),
      *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /M/MAP(60,60),IBLK,IENM1(6),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /N/INAME(3,4),IENM2(4,8),ISIDE(3,2),IBASE(2)
      COMMON /G/IARMZ(6),INDUZ,IXRYZ,IMAGZ,IMAGRZ,IAI3sZ,ISTAZ,INVIS(4),
     *ICLOZ,IDNNO,IPLANZ,IRUNZ,IDEVZ,IDRIZ,IMAXZ
      COMMON /S/IBPOB(8),IBPOE(25)
      COMMON /V/IWEB(2),IWEBZ,INVIS(4)
      IF(IBPOB(6).NE.0)RETURN
      WRITE(L,2)
...
20    FORMAT(' ENTER MAXIMUM WARP -- BETWEEN 2 AND 12')
      ...
      READ(L,9,ERR=73)DISTP
      IMAXZ=IFIX(DISTP)
      IF(IMAXZ.L.T.2.OR.IMAXZ.GT.12)GO TO 21
      WRITE(L,25)
...
42    FORMAT(' ENTER RANGE OF WEB')
      ...
      IF(IARMZ(4).EQ.0)GO TO 53
      WRITE(L,55)
55    FORMAT (' ENIER NUMBER OF FALSE IMAGES TO BE GENERATED  AT MOST 15')
      N=10
      READ(L,9,ERR=73)DISTP
      IMAGZ=IFIX(DISTP)
      IF(IMAGZ.LT.O.OR.IMAGZ.GT.15)GO TO 54
58    WRITE(L,57)
57    FORMAT (' FNIER RADIUS WITHIN WHICH THE IMAGES ARE TO BE GENERATED')
      N=11
      READ(L,91ERR=73)DISTP
      IMAGRZ=IFIX(DISTP)
      IF(IMAGRZ.LT.I.OR.IMAGRZ.GT.50)GO TO 58
