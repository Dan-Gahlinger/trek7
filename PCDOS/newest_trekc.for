...
109   CALL LAUREL(J)
100   CONTINUE
205   IF(WARF(IT).GT.TWARP(IT))WARP(IT)=TWARP(IT)
...
10020 IF(ITCOL(IT).EQ.IENTC(IT).AND.ITROW(IT).EQ.IENTR(IT))GO TO 600
      Q=WARP(IT)
      BERNG=ANG(ITR.DW(IT)-IENTR(IT),ITCOL(IT)-IENTC(IT))
      DO 224 I7=1,4
      IF(ICHOE(I7).NE.1)GO TO 224
      IF(I7.EQ.IT)GO TO 224
      MIN=0
      CALL DUNSEL(IENTR(IT),IENTC(IT),G,BERNG)
      IF(MIN.LT.2)GO TO 222
      MIN=MIN-2
      WRITE(L,223)(INAME(IQ0,17),IQ0=1,3)
...
224   CONTINUE
      ...
      IGNORE=0
        ...
        rtzro=0.0
        itvll=1
770   CALL HORTA(IENTR(IT),IENTC(IT),ITROW(IT),ITCOL(IT),G,
     *rtzro,itvll,rtzro,IGNORE,rtzro,itzro)
      J=0
...
535   FORMAT(' OUR COURSE IS BLOCKED BY A STAR')
      ...
      IVV=I8
        ...
        itzro=0
        rtzro=0.0
      CALL HORTA(IV1,IVV,IENTR(IT),IENTC(IT),rtvll,rtzro,itzro,
     *rtzro,itvllsrtzro,itzro)
...
550   DO 552 I7=1,5
      ...
      CALL RANDD(I8,1,12)
      WRITE(L,553)IS,LI2C(17),Ll2R(I7)
...
5323  IF(MIN.GT.18)GO TO 5322
      ...
      N=IV+4
      WRITE(N,16)(INAME(IQ0,IT),IQ0=1,3),(INAME(IQ0,MIN-14),IQ0=1,3),18,17
...
5333  FORMAT(1X,3A4,' COLLIDED WITH EAGLE',I3,' AT (',I2,',',I2,,')'/' EA
     *GLE',13,' DESTROYED'/' DAMAGE REPORT TO THE ',3A4)
226   CONTINUE
      CALL BOOM(IV)
...
C - DUNSEL -
C
      SUBROUTINE DUNSEL(JI,J2,01,02)
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(80)
      COMMON /L/IENTR(4),IENTC(4),IKLNR(6),IKLNC(8),IGLER(25),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /P/IPULL(4),IPUSH(4),IPULL(4),PUSH(4),IPULLR(4),IPULLC(4),
     *IPUSHR(4),IPUSHC(4)
...
102   FORMAT(' NACELLE',I2,' HAS RIPPED FREE OF THE SECONDARY HULL')
      IF(IGO(IT).EQ.2)IGO(IT)=1
      IF(MA(IT,J+28).GT.0)MANUM(IT)=MANUM(IT)-I
      MANUM(IT)=MANUM(IT)+1
      MA(IT,J+26)=9
      MA(IT,J+30)=9
     ...
     *rtvllsrtzro,it,zroortzro,itvll,rtzro,itzro)
...
C  SSSS  P      OOO   CCCC K   K
     ...
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      ...
      IWOW(X)=IFIX(ALOG((101.-X)*IVI)/FUDGE-1.8185)
      SPOCK=.FALSE.
74    CALL RANDO(IV1,1,100)
      LOCKT(IT)=0
      IF(IGO(IT).EQ.2)IGO(IT)=1
      IF(IVI.GT.7)GO TO 76
      IF(JS.EQ.0)GO TO 100
      WRITE(L,78)
...
761   N=3
     ...
     *itzro,RANG(IT),ANGLE(IT),J,AJUST,iitv11,itzro,KPLOT)
...
120   DO 121 I7=1,4
      ...
      I8=I7+4
      WRITE(I8, 122)(INAME(IQ0,IT),IQ0=1,3),(IBOLT(J,JS),J=21,4),
     *(ISIDE(IQ0,IV),IQ0=1,3),IBASE(IV)
122   FORMAT(1X,3A4,lX,4A4,' HAS HIT ',3A4,' STARBASE',I3)
121   CONTINUE
      IVV=IWOW(DFLCB(IV)/3.)
      CALL GRUP2(IVV,IV)
      SPOCK=.TRUE.
...
555   J=I7
     ...
     *rtvl1,itzro,itzro,itzro,itvlltitzro,itzro)
...
C
     ...
     *IGNORE,IO,IGOL(80)
      COMMON /D/DFLCT(4),DFLCK(8),DFLCB(2)
      COMMON /E/PHASR(4),TWARP(4),IPHOT(4),NDEAD(4),ISURR(4)
      COMMON /G/ZAP(4),ICCLA(4),IROWA(4),LOCK(4),ICOIL(4)
      COMMON /L/IENTR(4),IENTC(4),IKLNR(8),IKLNC(8),IGLER(25),IGLEC(25),
...
1011  FORMAT( ' MAIN COIL BURNOUT IN THE PHASER BANKS-'/' PHASERS ARE NOW
     *INOPERATIVE UNTIL REPAIRS ARE COMPLETE')
      GO TO 50
1010  N=DI(IENTR(IT),IENTC(IT),IRDWA(IT),ICOLA(IT))
      IF(N.EQ.0)GO TO 125
      J=IROWA(IT)-IENTR(IT)
...
128   IF(I.GT.14)GO TO 131
      ...
      I8=17+4
      WRITE(I8,133)(INAME(IQ0,IT),IQ0=1,3),(ISIDE(IQ0,1),IQ0=1,3),IBASE(I)
...
1013  IF(J.NE.I7)GO TO 780
      IF(IABS(IVI)/IVI.EQ.IABS(I8)/I8)GO TO 1016
      GO TO 780
1017  IF(FLOAT(IVI)/FLOAT(J).NE.FLOAT(I8)/FLOAT(I7))GO TO 780
      IF(IABS(IV1)/IV1.NE.IABS(I8)/I8)GO TO 780
1016  MIN=SGRT(FLOAT(I8**2+I7**2))
...
C
     ...
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      ...
      COMMON /N/INAME(3,4),IENM2(4,B),ISIDE(3,2),IBASE(2)
      COMMON /O/MA(4,33),K(4,14),NOMAP(4),MANUM(4),HIVEL(4,2),ITEMP(4),
     *NOMOV(4)
      COMMON /Q/IARMZ(6),INDUZ,IXRYZ,IMAGZ,IMAGRZ,IABSZ,ISTAZ,INVIZ(4)1
     *ICLOZ,IONNO,IPLANZ,IRUNZ,IDEVZ,IDRIZ,IMAXZ
      COMMON /R/IBPSC(4),IBPSB(8),IBPSS(2),IBPSE(25)
      COMMON /S/IBPOB(5),IBPOE(25)
      COMMON /T/ICHOE(4),ICHOS(8),ICHOB(2)
      COMMON /U/LAUNCH,NUMOUT,NUME(2)
      COMMON /V/IWEB(2),IWEBZ,INVIS(4)
      HARPO=.FALSE.
...
2     WRITE(L,3)
3     FORMAT('TRANSPORTER ROOMS DESTROYED- CANT BEAM A BOARDING PARTY')
5     RETURN
...
101   FORMAT(' BOARDING PARTY OPERATION NOW COMMENCING')
      DO 104 I=1,8
      IF(IROW.EQ.IKLNR(I).AND.ICOL.EQ.IKLNC(l))GO TO 105
104   CONTINUE
      DO 215 I=1,4
...
307   IF(.NOT.CHICO(NUM,ICOL,IROW,NDEAB(I),DFLCB(I),IBPSS(I),I5,NDEAD(IT
     *),4))RETURN
      HARPO=.TRUE.
      IF(NUM.LT.0.AND.NDEAB(I).EQ.0)WRITE(L,213)ISIDE(1,I),ISIDE(2,I)
     *IBASE(I)
...
C - TPRING -
C
C THIS SUBROUTING WAS HAND TYPED BY DG NOV. 17/1999
C
      SUBROUTINE TPRING(J)
      COMMON /A/IT,IS,II(2),IJ(2),I3,JS,ISHAK,NOSTOP
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
...
C - CHICO -
C
      LOGICAL FUNCTION CHICO(NUM,ICOL,IROW,IHORDE,DEFLT,ISID,ICNT,IFROM,J)
      LOGICAL ZEPPO
      COMMON /A/IT,IS,II(2),IJ(2),I3,JS,ISHAK,NOSTOP
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(80)
     ...
     *IBASR(2),IBABC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /N/INAME(2,4),IENM2(4,8),ISIDE(2,2),IBASE(2)
      COMMON /O/MA(4,33),K(4,14),NOMAP(4),MANUM(4),HIVEL(4,2),ITEMP(4),
     *NOMOV(4)
      COMMON /R/IBPSC(4),IBPSB(8),IBPSS(2),IBPSE(25)
      COMMON /T/ICHOE(4),ICHOS(8),ICHOB(2)
...
10    IF(J.NE.2)GO TO 1
      I8=IBPSC(I)+4
      IV=-NUM
      WRITE(I8,13)INAME(I,IT),INAME(2,IT),IV
13    FORMAT(' DO YOU WISH THE ',2A5,' TO BEAM OFF',I5,' OF YOUR CREW?')
      READ(I8,14)NA
14    FORMAT(A1)
      IF(NA.EQ.'Y')GO TO 1
      WRITE(L,15)INAME(1,I),INAME(2,I)
...
51    FORMAT(' CREW ON SHIP REINFORCED BY',I4)
      ...
      II(IS)=II(IS)+1
      IJ(IS)=IJ(IS)+1
      WRITE(I8,52)INAME(I,IT),INAME(2,IT)
52    FORMAT(' OUR SHIP HAS BEEN REINFORCED BY THE ',2A5,' YOU ARE BACK
     *IN CONTROL')
      GO TO 1
11    WRITE(I8,12)INAME(1,IT),INAME(2,IT)
...
140   I7=(120.-(MA(IT,JTK)+1)*DEFLT)*FLOAT(NUM)/100./DI(IENTC(IT),IENTR(
     *IT),ICOL,IROW)
      IF((DEFLT.LE.10.0.AND.MA(IT,JTK).LE.4).OR.(I7.GT.NUM))I7=(0.95*FLOAT(NUM))
      IFROM=IFROM-NUM
      IF(I7.LT.0)I7=0
      WRITE(L,101)I7
...
125   IF(ISID.NE.0)GO TO 127
      ...
      ICNT=ICNT+1
      GO TO 128
127   IF(J.EQ.4.AND.IHORDE.EQ.0)GO TO 126
      II(ISID)=II(ISID)-O
      IF(J.NE.3)GO TO 128
...
C
     ...
     *IGNORE,IO,IGOL(80)
      ...
      COMMON /NIINAME(2,4),IENM2(4,8),ISIDE(2,2),IBASE(2)
      COMMON /O/MA(4,33),K(4,14),NOMAP(4),MANUM(4),HIVEL(4,2),ITEMP(4),
     *NOMOV(4)
      COMMON /T/ICHOE(4),ICHOS(8),ICHOB(2)
      ZEPPO=.FALSE.
...
27    FORMAT(' HERE IS A LIST OF YOUR INTRUDER CONTROL COMANDS-'/
     ...
     *' ENEMY-HELD AREAS'/' S-ACTIVATE SELF-DESTRUCT MECHANISM'/
...
63    DO 75 IV=1,4
      ...
      I8=IV+4
      WRITE(I8,64)INAME(I,IT),INAME(2,IT)
64    FORMAT(1X,2A5,' SELF-DESTRUCT ROUTINE ACTIVATED')
75    CONTINUE
      DEFLT=-1.
      RETURN
36    WRITE(I8,65)
65    FORMAT(' OUR FORCEIS HAVE VANQUISHED THE INTRUDERS')
      CALL RANDO(IV,1,5)
      IF(IV.GT.1)GO TO 69
      WRITE(I8,66)
66    FORMAT(' BUT THE LAST INTRUDER DETONATED A BOMB BEFORE HIS CAPTURE
     *DAMAGE REPORT-')
      ...
      IF(IV.EQ.I)GO TO 67
      MIN=IV+4
      WRITE(MIN,68)INAME(1,I),INAME(2,I)
...
72    WRITE(I8,74)INAME(1,IT),INAME(2,IT),IFROM,DFLCT(IT),PHASR(IT),
...
269   CONTINUE
      ...
      GO TO 205
2040  RAD=PULL(IT)*10.
      BERNG=ANG(IENTR(IT)-IPULLR(IT),IENTC(IT)-IPULLC(IT)).*3,14159265/180.
...
C
     ...
     *IGNORE,IO,IGOL(80)
      COMMON /D/DFLCT(4),DFLCK(8),DFLCB(2)
      COMMON /L/IENTR(4),IENTC(4),IKLNR(8),IKLNC(8),IGLER 5),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /M/MAP(60,60),IBLK,IENM1(8),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /P/IPULL(4),IPUSH(4),PULL(4),PUSH(4),IPULLR(4),IPVLLC(4),
     *IPUSHR(4),IPUSHC(4)
      CURLY=.TRUE.
...
304   RAD=PUSH(IT)
      ...
      BERNG=ANG(IPUSHR(IT)-IENTR(IT),IPUSHC(IT)-IENTC(IT))*3.141592615/180.
...
206   CALL HORTA(IPUR,IPUC,itzro,itzro,RAD,BERNG,itzro,
     *itzro,IGNORE,itzro,itzro)
      ...
      WRITE(I81,272)INAME(1,MIN),INAME(2,MIN)
272   FORMAT(' MINE HIT ',2A5,'-DAMAGE REPORT-')
      GO TO 270
271   WRITE(I8I,273)IVV,INAME(1,MIN),INAME(2,MIN)
...
276   MIN=I71
      ...
      WRITE(I81,279)ISIDE(1,MIN),ISIDE(2,MIN),IBASE(MIN)
279   FORMAT(' MINE HIT ',2A5,' STARBASE',I3,'-DAMAGE REPORT-')
      GO TO 277
278   WRITE(I81,2BO)IVV,ISIDE(I,MIN),ISIDE(2,MIN),IBASE(MIN)
...
238   DO 242 I71=1,4
      ...
      I81=I71+4
      IF(IIV.EQ.1)00 TO 240
      WRITE(I81,239)IVV,I
239   FORMAT(' EAGLE',I3,' COLLIDED WITH EAGLE',I3/' BOTH EAGLES DESTROYED')
      GO TO 242
240   WRITE(I81,241)I
...
243   I71=I7
      I81=I8
        ...
        rtzro=0.0
        rtvll=1.5
      CALL HORTA(I71,IBI,IENTR(IT),IENTC(IT),rtvl1,
     *itzro,itzro,itzro,itvl1,itzro,itzro) IF(MIN.EQ.20)GO TO 243
226   MAP(IPUR,IPUC)=IBLK IF(IIV.EQ.2)GO TO 244 MAP(17,I8)=IM(IT)
      WRITE(L,245)I8,I7
