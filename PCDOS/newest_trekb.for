...
625   FORMAT(' NO WARP LEFT - BREAK OUT THE OARS')
      RETURN
15    IF(A.EQ.0.OR.B.EQ.0)G0 TO 150
      WARP(IT)=A
      ITCOL(IT)=B/100.
...
67210 WRITE(L,6704)
6704  FORMAT(' ENTER COMMANDS TO BE CANCELLED (P,T,B,Y,D, H FOR HELP)')
6708  READ(L,6705)(IGOL(I),I=1,6)
6705  FORMAT(6A1)
      IF(IGOL(1).NE.'H')GO TO 6706
      WRITE(L,6707)
...
6706  DO 6711 I7=1,6
      DO 6709 I=1,5
      IF(IGOL(I7).EQ.IGDK(I))GO TO (671,6712,674,6700,6701),1
6709  CONTINUE
      RETURN
...
672   FORMAT(' PHASER COMMAND CANCELLED')
      GO TO 6711
671   IF(LOCKT(IT).NE.I)GO TO 6711
      LOCKT(IT)=0
      IPHOT(IT)=IPHOT(IT)+1
      WRITE(L,673)
...
C
     ...
     *IGNORE,IO,IGOL(80)
      COMMON /E/PHASR(4),TWARP(4),IPHOT(4),NDEAD(4),ISURR(4)
      COMMON /H/ANGLE(4),RANG(4),LOCKT(4)
      DATA IBOLT/'PHOT','ON T','ORPE','DOES','DISR','UPTO','RS'
...
71    DISTP=ANGLE(IT)
      ...
      ANGLE(IT)=A
      GO TO 71
7100  WRITE(L,3)IPHOT(IT),(IBOLT(I,JS),I=1,4)
...
C
      ...
      COMMON /C/L,A,B,I,NA,IV,i7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(80)
      COMMON /D/DFLCT(4),DFLCK(5),DFLCB(2)
      COMMON /M/MAP(60,60),IBLK,IENM1(8),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /P/IPULL(4),IPUSH(4),PULL(4),PUSH(4),IPULLR(4),IPULLC(4),
     *IPUSHR(4),IPUSHC(4)
...
56    FORMAT('THIS WILL USE ', F11.7,'UNITS OF DEFLECTOR SHIELD ENERGY')
      GO TO 57
330   IF(A.NE.0)GO TO 37
      WRITE(L,34)
...
35    IPUSH(IT)=1
      WRITE(L,37)
37    FORMAT(' DEFLECIUKS LOCKED ON TARGET')
      RETURN
      WRITE(L,265)
...
C- WE'RE WAITING FOR
      SUBROUTINE GODOT
      DIMENSION LEN(31),LEN2(31)
      COMMON /A/IT,IS,II(2),IJ(2),I3,JS,ISHAK,NOSTOP
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG
     *IGNORE,IO,IGOL(80)
      COMMON /I/IDNK(8),ISPOK(8),IDN(4),ISPOT(4)
      COMMON /L/IENTR(4),IENTC(4),IKLNR(E3),IKLNC(8),IGLER(25),IGLEC(25),
     *IBASR(2),IBABC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /M/MAP(60,60),IBLK,IENM1(8),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /O/MA(4,33),K(4,14),NOMAP(4),MANUM(4),HIVEL(4,2),ITEMP
     *NOMOV(4)
      COMMON /G/IARMZ(6),INDUZ,IXRYZ,IMAGZ,IMAGRZ,IABSZ,ISTAZ,INVIZ
...
552   LEN(J-IV+1)=J-(J/10)*10
      WRITE(L,5175)(LEN(J),J=1,IVV-IV+l)
5175  FORMAT(3X,31I1)
      DO 5173 J=1,I8-I7+1
      IV1=I8-J+1
      JTK=IV1/10
...
103   LEN2(JTK)=IBLK
100   CONTINUE
5173  WRITE(L,5172)IVI,(LEN2(I),I=1,JTK)PNB,NA
5172  FORMAT(I3,33A1)
...
C
      ...
      COMMON /C/L,A,B,I,NA,IV,T7,IB,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(80)
     ...
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      ...
      COMMON /Q/IARMZ(6),INDUZ,IXRYZ,IMAGZ,IMAGRZ,IABSZ,ISTAZ,TNVIZ(4),
...
587   FORMAT(22X,'DEFLECTORS POSITION'/1X,3A4,13X,F5.1,4X,'(',I2,',',I2,
     *')',5X,'CREW REMAINING=',I4)
      DO 770 N=1,4
      IF(ICHOE(N).EQ.O.DR.ICHOE(N).EQ.3)GO TO 770
...
770   CONTINUE
      ...
      WRITE(L,773)(ISIDE(IGO,N),IQ0=1,3),IBASE(N),DFLCB(N),IBASC(N),
     *IBASR(N)
...
755   FORMAT(1X,4A4,7X,F5.1,4X,f(',I2,',',I2,')')
      GO TO 760
...
756   FORMAT(1X,4A4,7X,'UNKNOWN (',I2,',',I2,')')
      GO TO 760
764   IF(IARMZ(4).EQ.I.AND.IMAGZ.GT.0)GO TO 91
      IF(IARMZ(5).EQ.I.AND.INVIZ(IT).EQ.0)00 TO 91
      WRITE(L,755)(IENM2(I,6),I=1,4),DFLCK(6),IKLNC(6),IKLNR(6)
      GO TO 760
91    WRITE(L,757)(IENM2(1,6)#I=1,4)
760   CONTINUE
      RETURN
      END
...
4     FORMAT(' MAP - PRINT A MAP OF YOUR SURROUNDINGS'/
     ...
     *' RADIO   USE SUBSPACE RADIO TO CONTACT OTHER MANUALLY-OPERATED SHIPS'/
...
37    FORMAT(' ENEMY',12X,'WARP FIREPOWER'//' KLINGONS',9X,'7-9 450-110
     *0'/' ROMULANS's9X,'7-9 300-1000'/' THOLIANS',9X,'5-10 600-900'/
...
C
     ...
     *IGNORE,IO,MES(80)
      COMMON /E/PHASR(4),TWARP(4),IPHOT(4),NDEAD(4),ISURR(4)
      COMMON /I/IGNK(8),ISPOK(8),ION(4),ISPOT(4)
      COMMON /J/KODE(2,8),STATIC(4)
      COMMON /L/IENTR(4),8IENTC(4),IKLNR(8),IKLNC(8),IGLER(25),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /M/MAP(60,60),IBLK,IENM1(8),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /N/INAME(3,4),IENM2(4,8),ISIDE(3,2),IBASE(2)
      COMMON /R/IBPSC(4),IBPSB(8),IBPSS(2),IBPSE(25)
...
34    FORMAT(' MESSAGE FROM THE ',3A4/IX,SOAI)
      GO TO 35
...
C - ORWELL -
C
      SUBROUTINE ORWELL.(IBORD,NUMPIROW,ICOL)
      LOGICAL CYRANO
      COMMON /A/IT,IS,II(2),IJ(2),I3,JS,ISHAK,NOSTOP
      COMMON /C/L,A,B,I,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(6)
...
C       -- DONALD ECCLESTONE AND DAN GAHLINGER SUBPROGRAM FOR T R E K7
C
...
C       SSSS   SSSS  OOO    T     T
     ...
     *IGNORE,IO,IGOL(80)
      ...
      COMMON /L/IENTR(4),IENTC(4),IKLNR(8),IKLNC(8),IGLER(25),IGLEC(25),
     *IBASR(2),IBASC(2),LI2(4),LI2R(5),LI2C(5),IGO(4),MINES
      COMMON /M/MAP(60,60),IBLK,rENMI(S),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /R/IBPSC(4),IBPSB(8),IBPSS(2),IBPSE(25)
      COMMON /T/ICHOE(4),ICHOS(8),ICHOB(2)
...
642   IF(ZAP.GT.DFLCT(IT))GO TO 646
      ...
      PHASR(IT)=PHASR(IT)+ZAP*100.
      GO TO 6320
643   IF(ZAP.GT.FLOAT(IPHOT(rT)))GO TO 646
...
671   IF(ICHOE(I).EQ.0.OR.ICHOE(I).EQ.3)GO TO 646
      ...
      IF(ZAP.GT.DFLCT(IT))GO TO 646
      IF(DFLCT(I)+7-AP.GT.100.)ZAP=100.-DFLCT(l)
...
C
      ...
      COMMON /A/IT,IS,II(2),IJ(2),I3,JS,ISHAK,NOSTOP
      COMMON /C/L,A,B.IQ,NA,IV,I7,I8,N,DISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *,IGNORE,IO,IGOL(80)
      COMMON /D/DFLCT(4),DFLCK(8)oDFLCB(2)
...
C
     ...
     *IPUSHR(4),IPUSHC(4)
      ...
      GO TO 59
58    WRITE(L,21)DISTP
21    FORMAT(' RANGE OF BEAM IS, F4. 0, UNITS'/' ENTER TARGET COORDINATES:  '$)
      A=0
...
41    FORMAT(' IF THE TARGET IS A SHIP-'/' THE AMOUNT OF ENERGY (E) ENTE
     ...
     *DED FOR EACH UNIT THIS OBJECT IS PULL-ED IN,//, IF THE TARGET IS EM
...
21    FORMAT(' IF WE GO WARP 10, PROB. OF ESCAPING WEB=14.2657142857%'/
     ...
     *' IF WE FIRE A TORPEDO, PROB. THAT IT WILL PENETRATE WEB=25%'/5X1
...
C- THE WEED OF CRIME BEARS BITTER FRUIT
      ...
      COMMON /C/L,AB,I,NA,IV,I7,I8,NDISTP,AJUST,MIN,ISTAT,JTK,KOENIG,
     *IGNORE,IO,IGOL(80)
      COMMON /K/IWHO(8),DIST(8),KILLZ,KILLR,KILLD,KILLG,IGOCO(13)
      COMMON /M/MAP(60,60),IBLK,IENMI(8),IEE(4),IGLE,IM(4),III,ISTAR
      COMMON /N/INAME(2,4),IENM2(4,8),ISIDE(2,2),IBASE(2)
      COMMON /R/IBPSC(4),IBPSB(8),IBPSS(2),IBPSE(25)
      COMMON /T/ICHOE(4),ICHOS(6),ICHOB(2)
      WRITE(L,1)
I     FORMAT(' ENTER SHIP LETTER (''*'' FOR STARBASE): '$)
      READ(L,2)NA
2     FORMAT(A1)
...
6     IF(IBPSS(I).NE.IS)GO TO 501
      ...
      IV=IBPSC(I8)+4
      IF(J.EQ.0)GO TO 8
      WRITE(IV,9)ISIDE(1,I),ISIDE(2,I),I7
9     FORMAT(1X,2A5,' STARBASE', 13, ' HAS CEASED FIRING')
      GO TO 7
8     WRITE(IV,10)ISIDE(I,I),ISIDE(2,I),I7
...
106   IF(IBPSB(I).NE.IS)GO TO 501
      ...
      IF((IBPSC(IS)+I)/2.NE.IS)GO TO 107
      IV=IBPSC(IB)+4
      IF(J.EQ.0)GO TO 108
      WRITE(IV,109)(IENM2(N,I),N=1,
